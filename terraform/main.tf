...
# (DEMO) Políticas gestionadas: enrutar /api al backend sin cachear y reenviando todo.
...
resource "aws_cloudfront_distribution" "frontend" {
  ...
  # (DEMO) Origen del backend .NET en Azure (AKS dev) para enrutar /api por HTTPS.
  origin {
    domain_name = "sales-api-backend-dev.westus3.cloudapp.azure.com"
    origin_id   = "backend-azure"
  ...
  # (DEMO) /api/* → backend Azure (sin caché; reenvía método, query, body y headers incl. Authorization).
  ordered_cache_behavior {
    path_pattern             = "/api/*"
    target_origin_id         = "backend-azure"
